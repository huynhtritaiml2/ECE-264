...
int main(int argc, char ** argv) {
  ...
  BoxFilter * filter = readFilter(fopen("blur.filter", "r"));

  BMPImage * testImage = malloc(sizeof(BMPImage));
  copyImage(actualImage, testImage);
  apply(actualImage, testImage, *filter, 31);
  
  freeImage(actualImage);
  free(filter);

  actualImage = testImage;
  writeImage(testImage, fopen("result.bmp", "w"));
